...
if args.ip:
  print "ip pkt chosen with len=" + str(len(args.ip))
  dstip = args.ip[0]
  if len(args.ip) > 1:
    srcip = args.ip[1]
  if len(args.ip) > 2:
...
if args.ip6:
    print "ip6 pkt chosen with args=" + str(len(args.ip6))
    dstip6 = args.ip6[0]
...
print "len(dstip)=" + str(len(dstip))
print "len(dstip6)=" + str(len(dstip6))

for dst_port in range(0,count):
  #if dstip and len(dstip6) == 0:
  if args.ip:
   if dstip:
     print "should not come here for ip6 1"  
     if vid != 0:
       if proto == "udp":
   ...
   #else:
   #  print "should not come here for ip6 2"  
   #  if vid != 0:
